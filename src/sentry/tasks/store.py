...
from sentry.eventstore.tmpstore import event_tmpstore
...
def _do_preprocess_event(cache_key, data, start_time, event_id, process_task, project):
    ...
    if cache_key and data is None:
        data = event_tmpstore.get(cache_key)
...
def _do_symbolicate_event(cache_key, start_time, event_id, symbolicate_task, data=None):
    ...
    if has_changed:
        cache_key = event_tmpstore.store(data)
...
def create_failed_event(
    ...
    event_tmpstore.delete_by_key(cache_key)
